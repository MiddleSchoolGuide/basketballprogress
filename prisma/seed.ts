...
async function main() {
  ...
  // Create goals
  const goals = [
    { metricName: 'leftHandControl', baselineValue: 2, targetValue: 5, targetDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000) },
    { metricName: 'formShooting', baselineValue: 4, targetValue: 6, targetDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000) },
    { metricName: 'freeThrowPct', baselineValue: 30, targetValue: 50, targetDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000) },
    { metricName: 'spotShootingPct', baselineValue: 30, targetValue: 42, targetDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000) },
    { metricName: 'footwork', baselineValue: 3, targetValue: 6, targetDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000) },
    { metricName: 'stopPopSpeed', baselineValue: 4, targetValue: 6, targetDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000) },
  ]

  for (const goal of goals) {
    await prisma.goal.create({ data: { ...goal, playerId: player.id } })
  }

  // Create drill checklist items
  const drills = [
  ...
  // Create sample sessions showing progression
  const sessionData = [
    {
      daysAgo: 14,
  ...
  console.log('✅ Seed complete — player, 5 sessions, goals, and drills created.')
}
